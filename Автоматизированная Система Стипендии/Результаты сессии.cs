...
{
    public partial class Результаты_сессии : Form
    {
        static List<string[]> authDate = new List<string[]>();
        static string mybdpath;
        public Результаты_сессии(string bdpath)
        {
            mybdpath = bdpath;
            InitializeComponent();
            ...
            connection.Open();
            command = new OleDbCommand("SELECT Фамилия, Имя, Отчество, Группы.Группа FROM Студенты INNER JOIN Группы ON Группы.ID = Студенты.[ID группы] WHERE Студенты.[ID сессии] = 1", connection);
            connection.Close();
            adapter.SelectCommand = command;
